...
static bool try_one_request(Conn *conn) {
    ...
    // rewritten with the new error string handling
    // parse the request 
...
/* take the nearest timer from the list and use it to calculate 
 the timeout value of poll 
*/ 
...
// at each iteration of the event loop, the list is checked to fire timers in due time
static void process_timers() {
    ...
    while (!dlist_empty(&g_data.idle_list)) {
        ...
        if (next_us >= now_us + 1000) {
            // not ready, the extra 1000ys is for the ms resolution of poll()
            break;
        }
